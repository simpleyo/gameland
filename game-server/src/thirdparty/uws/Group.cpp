...
template <bool isServer>
void Group<isServer>::startAutoPing(int intervalMs, const std::string& userMessage) {
...
template <bool isServer> static void _default_connectionHandler          (WebSocket<isServer> *, HttpRequest)                                 {}
template <bool isServer> static void _default_transferHandler            (WebSocket<isServer> *)                                              {}
template <bool isServer> static void _default_messageHandler             (WebSocket<isServer> *, char *message, size_t length, OpCode opCode) {}
template <bool isServer> static void _default_disconnectionHandler       (WebSocket<isServer> *, int code, char *message, size_t length)      {}
template <bool isServer> static void _default_pingHandler                (WebSocket<isServer> *, char *, size_t)                              {}
//template <bool isServer> void _default_pongHandler                (WebSocket<isServer> *, char *, size_t)                              {}
template <bool isServer> static void _default_httpConnectionHandler      (HttpSocket<isServer> *)                                             {}
template <bool isServer> static void _default_httpRequestHandler         (HttpResponse *, HttpRequest, char *, size_t, size_t)                {}
template <bool isServer> static void _default_httpDataHandler            (HttpResponse *, char *, size_t, size_t)                             {}
template <bool isServer> static void _default_httpCancelledRequestHandler(HttpResponse *)                                                     {}
template <bool isServer> static void _default_httpDisconnectionHandler   (HttpSocket<isServer> *)                                             {}
//template <bool isServer> void _default_httpUpgradeHandler         (HttpSocket<isServer> *, HttpRequest)                                {} // Esta no hay que definirla.
template <bool isServer> static void _default_errorHandler               (typename Group<isServer>::errorType)                                {}
...
Group<isServer>::Group(int extensionOptions, unsigned int maxPayload, Hub *hub, uS::NodeData *nodeData) : uS::NodeData(*nodeData), maxPayload(maxPayload), hub(hub), extensionOptions(extensionOptions) {
    ...
    //connectionHandler           = _default_connectionHandler          <isServer>;
    //transferHandler             = _default_transferHandler            <isServer>;
    //messageHandler              = _default_messageHandler             <isServer>;
    //disconnectionHandler        = _default_disconnectionHandler       <isServer>;
    //pingHandler                 = _default_pingHandler                <isServer>;
    ////pongHandler                 = _default_pongHandler                <isServer>;
    //httpConnectionHandler       = _default_httpConnectionHandler      <isServer>;
    //httpRequestHandler          = _default_httpRequestHandler         <isServer>;
    //httpDataHandler             = _default_httpDataHandler            <isServer>;
    //httpCancelledRequestHandler = _default_httpCancelledRequestHandler<isServer>;
    //httpDisconnectionHandler    = _default_httpDisconnectionHandler   <isServer>;
    ////httpUpgradeHandler          = _default_httpUpgradeHandler         <isServer>;
    //errorHandler                = _default_errorHandler               <isServer>;

    // Todo esto no funciona. Provoca errores en el destructor.
    // Parece que el error tiene que ver con asignar funciones lambdas a std:function.
...
template <bool isServer>
void Group<isServer>::close(int code, const char *message, size_t length) {
